...
{
    public class channel_3 : IChannel
    {
        ...
        private string Path;
        public channel_3(IMessagesDal messagesDal, string path)
        {
        ...
        public async Task<int> PostXml(string url, string content, long mesID, long sysHash)
        {
            ...
            if (response.StatusCode == HttpStatusCode.NoContent)
            {
                ...
                if (await _messagesDal.UpdateSuccessMessageAsync(mesID, sysHash))
                {
                    ...
                    if (!File.Exists(Path))
                    {
                        ...
                        {
                            sw.WriteLine(DateTime.UtcNow.ToString("O") + " " + mesID + " Chnl03 OK");
                        }
                    }
                    else
                    {
            ...
            else
            {
                ...
                if (!File.Exists(Path))
                {
                    using (var sw = File.CreateText(Path))
                    {
                        sw.WriteLine(DateTime.UtcNow.ToString("O") + " " + mesID + "Chnl03 Content:" + content + " PostXml Error:" + err.Text);
                    }
                }
                else
                {
