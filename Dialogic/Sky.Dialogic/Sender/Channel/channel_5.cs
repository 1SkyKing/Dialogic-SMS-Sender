...
{
    public class channel_5 : IChannel
    {
        ...
        private string Path;
        public channel_5(IMessagesDal messagesDal, string path)
        {
        ...
        public async Task<int> PostXml(string url, string content, long mesID, long sysHash)
        {
            ...
            if (response.StatusCode == HttpStatusCode.NoContent)
            {
                ...
                if (await _messagesDal.UpdateSuccessMessageAsync(mesID, sysHash))
                {
                    ...
                    if (!File.Exists(Path))
                    {
                        ...
                        {
                            sw.WriteLine(DateTime.UtcNow.ToString("O") + " " + mesID + " Chnl05 OK");
                        }
                    }
                    else
                    {
            ...
            else
            {
                ...
                if (!File.Exists(Path))
                {
                    using (var sw = File.CreateText(Path))
                    {
                        sw.WriteLine(DateTime.UtcNow.ToString("O") + " " + mesID + "Chnl05 Content:" + content + " PostXml Error:" + err.Text);
                    }
                }
                else
                {
